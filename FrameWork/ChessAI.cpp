...
// 모든 가능한 이동을 찾는 함수
std::vector < MoveLogic::Move > ChessAI::GetAllPossibleMoves(
        ...
        for (int col = 0; col < BOARD_SIZE; ++col) {
            ...
            if (currentPiece != nullptr && currentPiece -> getAlive()) {
                ...
                if (currentPiece -> getOwner() == AI) {
                    ...
                    std::vector < std::pair<int, int> > legalDestinations = currentPiece -> getLegalMoves(
                        & boardctrl
                    );
    ...
    sprintf_s(
        debugMsg,
        "AI 기물 수: %d, 합법적 이동 수: %d\n",
        aiPieceFoundCount,
        totalLegalMovesCount
    );
...
MoveLogic::Move ChessAI::GetBestMove(BoardController & virtualBoard) {
    ...
    std::vector < MoveLogic::Move > possibleMoves = GetAllPossibleMoves(
        virtualBoard
    );
