...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
	...
	*/
	...
	//sound.g_pSoundManager = new CSoundManager();					//으아아아아악
	//sound.g_pSoundManager->Initialize(g_hWnd, DSSCL_PRIORITY); // 여기 주석 처리했음 ! 풀어 야 함 !  살려 줘 !@# ㅅ 당...당장 밖으로 낙 .. .  이제 안전합니다
	//sound.g_pSoundManager->SetPrimaryBufferFormat(2,22050,16); // 모두 여기서 살아가도로로로로로로로로로로곡합시다 
	...
	g_Mng.chap[GAME] = Game::getInstance();
	g_Mng.chap[OVER] = new Over;
	/////////////////////////////////////////

	Game* gameInstance = static_cast<Game*>(g_Mng.chap[GAME]);
	if (gameInstance)
	{
		gameInstance->SetWindowHandle(g_hWnd);
	}
	else {
		return 1;
	}

	Menu* menuInstance = static_cast<Menu*>(g_Mng.chap[MENU]);
	if (menuInstance)
	{
		menuInstance->Init(g_hWnd);  // Init 함수에서 hWnd 저장
	}
	else {
		return 1;
	}
	...
	while( msg.message != WM_QUIT )
	{
		...
		else
		{
			...
			g_Mng.chap[g_Mng.n_Chap]->Draw(hdc);
...
LRESULT CALLBACK WndProc( HWND g_hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_PAINT:
        ...
        Game::getInstance()->Draw(hdc);  // hdc를 Game으로 전달
	...
	case WM_LBUTTONDOWN:
		...
		POINT boardPos = pieceCtrl.HandleMouseClick(pt.y, pt.x);

		Gmanager.m_SysTem.m_Save1 = boardPos.y;
		Gmanager.m_SysTem.m_Save2 = boardPos.x;

		pieceCtrl.PieceMove(pt.y, pt.x);
		break;
		}
	case WM_DESTROY:
         PostQuitMessage( 0 );
